...
def load_all_data():
    ...
    data_dict = {}
    files = [f for f in os.listdir(data_file_path) if f.endswith('.csv') and f != "original.csv"]
...
def create_individual_category_pie_charts(data_dict):
    ...
    for category, df in data_dict.items():
        ...
        plt.title(f"Overall Response Distribution for {category.capitalize()}", fontsize=14, weight="bold")
        plt.axis("off")
        plt.text(
            0.5,
            -0.1,
...
def create_total_pie_chart(data_dict):
    ...
    plt.title("Overall Response Distribution (All Categories)", fontsize=14, weight="bold")
    plt.axis("off")
    plt.text(
        0.5,
        -0.1,
